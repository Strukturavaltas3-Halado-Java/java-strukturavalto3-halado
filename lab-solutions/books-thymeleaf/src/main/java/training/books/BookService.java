...
public class BookService {
    ...
    @Transactional
    public BookWithGenreDto updateBookWithGenre(long id, UpdateBookWithGenreCommand command) {
        Book book = bookRepository.findById(id).orElseThrow(()->new BookNotFoundException(id));
        book.setGenre(Genre.valueOf(command.getGenre()));
        return booksConverter.convertBookWithGenre(book);
    }
}
